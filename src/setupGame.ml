...
let validateBlueprints playerBlueprints =
    let notEnoughPlayers = if List.length playerBlueprints < 2 then [NotEnoughPlayers] else []
    and characterLimit = 50 in
    let tooManyCharacters =
        playerBlueprints
        |> List.filter (fun (Name name, _) -> String.length name > characterLimit)
        |> List.map (fun x -> NameExceededCharacterLimit (characterLimit, x)) in
    ...
    let duplicatedColor =
        ...
        |> List.map (fun (x, (y)) -> DuplicatedColor (x, y)) in

    let errors = 
        notEnoughPlayers
        |> List.append tooManyCharacters
        |> List.append duplicatedColor in
...
let startWithSeedGame seed playerBlueprints =
    ...
    let board = 
        ...
        let shuffledNumberTokens = BoardPieces.numberTokens
                                    |> Linq.sortByChar (fun x -> x.letter) in
        ...
        let terrainTiles = BoardPieces.terrainHexes
                            |> shuffle
                            |> buildTerrainTiles shuffledNumberTokens
                            |> List.map (fun x -> Terrain x) in
        ...
        and polarToCartesian x = 
            ...
            |> List.map (fun (r, t) -> (round2 r, round2 t )) in

        let createAllCartesianPoints (x, y) =
            createAllPolarPoints (x, y)
            |> polarToCartesian
        ...
        and cornerPoints =
            ...
            |> List.concat
            |> List.map (fun (x, y) -> { x=x;y=y;item=Corner Empty; })
        ...
        and edgePoints =
            [
                ...
                (hexCenterToEdge *. 3.5, hexEdgeLength *. (Float.sqrt(3.0) /. 2.0) +. hexCenterToEdge *. 1.5);
                (hexCenterToEdge *. 3.5, hexEdgeLength *. (Float.sqrt(3.0) /. 2.0));
                (hexCenterToEdge *. 2.5, hexEdgeLength *. (Float.sqrt(3.0) /. 2.0) +. hexCenterToEdge *. 1.5);
            ...
            |> List.map createAllCartesianPoints
            |> List.concat
            |> List.map (fun (x, y) -> { x=x;y=y;item=Edge Empty; }) in
    ...
    *)
    ...
    validateBlueprints playerBlueprints
    |> Result.map createGame

let startGame playerBlueprints =
    startWithSeedGame 1000 playerBlueprints